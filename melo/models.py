...
class SynthesizerTrn(nn.Module):
    ...
    def load_trt_engine(self, engine_paths):
        for engine_name in engine_paths:
            self.trt_engines[engine_name] = load_engine(
                engine_paths[engine_name]
            )
            # self.trt_engine_contexts[engine_name] = self.trt_engines[engine_name].create_execution_context()

    def __is_trt_existed(self):
        r = self.trt_engines.get("dec", None) is not None
        return r
    ...
    def infer_trt(
        ...
        if not self.__is_trt_existed():
            if self.trt_engine_path is not None:
                self.load_trt_engine(self.trt_engine_path)
            else:
                raise(ValueError("trt engine is not loaded"))
        ...
        dec_inputs = prepare_dec_inputs((z * y_mask)[:, :, :max_len], g, device=device)
        context = self.trt_engines["dec"].create_execution_context()


        run_trt_engine(
            context,
            self.trt_engines["dec"],
            dec_inputs
        )

        o = dec_inputs["outputs"]["o"]
